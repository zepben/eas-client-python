...
from .exceptions import GraphQLClientError, GraphQLClientGraphQLMultiError, GraphQLClientHttpError, GraphQLClientInvalidMessageFormat, GraphQLClientInvalidResponseError
try:
...
class AsyncBaseClient:
    ...
    def __init__(self, url: str='', headers: Optional[dict[str, str]]=None, http_client: Optional[httpx.AsyncClient]=None, ws_url: str='', ws_headers: Optional[dict[str, Any]]=None, ws_origin: Optional[str]=None, ws_connection_init_payload: Optional[dict[str, Any]]=None) -> None:
        self.url = url
    ...
    async def execute_ws(self, query: str, operation_name: Optional[str]=None, variables: Optional[dict[str, Any]]=None, **kwargs: Any) -> AsyncIterator[dict[str, Any]]:
        ...
        async with ws_connect(self.ws_url, subprotocols=[Subprotocol(GRAPHQL_TRANSPORT_WS)], **merged_kwargs) as websocket:
            ...
            try:
                await asyncio.wait_for(self._wait_for_connection_ack(websocket), timeout=5.0)
    ...
    async def _send_subscribe(self, websocket: ClientConnection, operation_id: str, query: str, operation_name: Optional[str]=None, variables: Optional[dict[str, Any]]=None) -> None:
        payload_inner: dict[str, Any] = {'query': query, 'operationName': operation_name}
        if variables:
            payload_inner['variables'] = self._convert_dict_to_json_serializable(variables)
        payload: dict[str, Any] = {'id': operation_id, 'type': GraphQLTransportWSMessageType.SUBSCRIBE.value, 'payload': payload_inner}
        await websocket.send(json.dumps(payload))
